...
if [[ "$1" = "#"* ]]; then # this is a color
    rand/generate_colors_material.py --color "$1" \
    ...
    rand/generate_colors_material.py --path "$1" \
    --mode "$lightdark" --scheme "$materialscheme" --transparency "$transparency" \
    --termscheme $terminalscheme --blend_bg_fg \
    ...
    cat rand/pywal_to_material.scss >> "$CACHE_DIR"/user/generated/material_colors.scss
    if [ "$2" = "--apply" ]; then
